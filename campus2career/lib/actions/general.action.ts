...
export async function getFeedbackByInterviewId(
    ...
    const response = await fetch(`http://localhost:5000/api/feedback?interviewId=${interviewId}&userId=${userId}`);
    if (!response.ok) return null;
...
export async function getLatestInterviews(
    ...
    const response = await fetch(`http://localhost:5000/api/interviews/latest?userId=${userId}&limit=${limit}`);
...
export async function getInterviewsByUserId(
    ...
    const response = await fetch(`http://localhost:5000/api/interviews?userId=${userId}`);
    if (!response.ok) return [];
    return await response.json();
}
