...
export default function CompaniesManagement() {
  ...
  const handleUpdateCompany = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await apiCall('/api/companies/${editingCompany._id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const confirmDeleteCompany = async () => {
    ...
    try {
      ...
      const response = await apiCall('/api/companies/${companyToDelete._id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleDocumentUpload = async () => {
    ...
    try {
      ...
      for (const fileData of uploadedFiles) {
        ...
        const response = await apiCall('/api/companies/${selectedCompany._id}/documents`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`
          },
  ...
  const confirmDeleteDocument = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await apiCall('/api/companies/${company._id}/documents/${documentId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
