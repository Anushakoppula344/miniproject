...
export default function ForumPage() {
  ...
  const handleLike = async (questionId: string) => {
    try {
      const response = await fetch(`http://localhost:5000/api/questions/${questionId}/like`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleBookmark = async (questionId: string) => {
    try {
      const response = await fetch(`http://localhost:5000/api/questions/${questionId}/bookmark`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          // 'Authorization': `Bearer ${localStorage.getItem('token')}` // Temporarily disabled for testing
