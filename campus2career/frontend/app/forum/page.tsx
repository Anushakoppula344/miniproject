...
export default function ForumPage() {
  ...
  const handleBookmark = async (questionId: string) => {
    try {
      const response = await apiCall(`/api/questions/${questionId}/bookmark`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          // 'Authorization': `Bearer ${localStorage.getItem('token')}` // Temporarily disabled for testing
  ...
  const handleReply = async (questionId: string) => {
    ...
    try {
      const response = await apiCall(`/api/questions/${questionId}/answers`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleViewQuestion = async (questionId: string) => {
    try {
      ...
      const response = await apiCall(`/api/questions/${questionId}`, {
        headers: {
          'Content-Type': 'application/json'
        }
      });
